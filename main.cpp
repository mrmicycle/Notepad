...
int main()
{
        ...
        if (_kbhit())
        {
            ...
            if (x == 0) 
            {
                ...
                if (x == 59)
                {
                    ...
                    getline(cin, np.filename);// this will save the filename to notepad before opening
                    system("cls");//clear title screen
                    np.gotoxy(0, 0);// reset cursor
                    np.ShowConsoleCursor(true);// show cursor
                    break;//exit loop
                }
            }
            else
    ...
    // row 29 is the bottom of my default terminal. (y=29)
    /*while (1)
    {
        gotoxy(0,28);
        cout << "hello is this at the bottom?";
        gotoxy(0, 0);
        Sleep(100000000000);
    }*/
...
//    // initiate Notepad and all variables
